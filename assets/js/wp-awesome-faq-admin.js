(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.jltwpafaq-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.jltwpafaq-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.jltwpafaq-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.jltwpafaq-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: JLTWPAFAQCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jltwpafaq_recommended_activate_plugin",
            file: file,
            nonce: JLTWPAFAQCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jltwpafaq_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTWPAFAQCORE.recommended_nonce
        },
