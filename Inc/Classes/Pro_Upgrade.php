...
use JLTWPAFAQ\Inc\Classes\Notifications\Base\Date;
...
class Pro_Upgrade {
	...
	public function __construct() {
		$this->slug = Helper::jltwpafaq_slug_cleanup();
	...
	public function dashboard_widget() {
		wp_add_dashboard_widget(
			'jltwpafaq_dashboard_widget',                          // Widget slug.
			esc_html__( 'wp-awesome-faq News & Updates', 'wp-awesome-faq' ), // Title.
		...
		$example_widget_backup = array( 'jltwpafaq_dashboard_widget' => $default_dashboard['jltwpafaq_dashboard_widget'] );
		unset( $default_dashboard['jltwpafaq_dashboard_widget'] );
	...
	public function dashboard_widget_render() {
		...
		echo '<div class="jltwpafaq-rss-widget">';

		if ( wp_validate_boolean( $this->get_content( 'is_campaign' ) ) ) { ?>

			<div class="jltwpafaq-dashboard-promo" style="--jltwpafaq-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				<a target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
					<img src="<?php echo esc_url( $this->get_content( 'image_url' ) ); ?>" alt="wp-awesome-faq Promo Image" style="width: 100%; height: auto;">
				</a>

				<a class="jltwpafaq-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
		...
		if ( ! empty( $maxitems ) ) {
			...
			foreach ( $rss_items as $item ) {
				...
				$item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'wp-awesome-faq' );
		...
		<div class="jltwpafaq-dashboard_footer">
			<ul>
				<li class="jltwpafaq-overview__blog">
                    <a href="https://jeweltheme.com/blog" target="_blank">
                        Blog
                        <span class="screen-reader-text">
                            <?php echo esc_html__( '(opens in a new window)', 'wp-awesome-faq' ); ?>
                        </span>
                        <span aria-hidden="true" class="dashicons dashicons-external"></span>
                    </a>
                </li>
                <li class="jltwpafaq-overview__help">
                ...
                <li class="jltwpafaq-overview__upgrade">
		...
		<style>
			...
			.jltwpafaq-rss-widget .hndle.ui-sortable-handle img {
				margin: -5px 10px -5px 0;
			}

			.jltwpafaq-rss-widget .jltwpafaq-dashboard_footer {
				margin: 0 -12px -12px;
				padding: 0 12px;
				border-top: 1px solid #eee;
			}

			.jltwpafaq-rss-widget .jltwpafaq-dashboard_footer ul {
				display: flex;
				list-style: none;
			}

			.jltwpafaq-rss-widget .jltwpafaq-dashboard_footer ul li:first-child {
				padding-left: 0;
				border: none;
			}

			.jltwpafaq-rss-widget .jltwpafaq-dashboard_footer li {
				padding: 0 10px;
				margin: 0;
				border-left: 1px solid #ddd;
			}

			.jltwpafaq-rss-widget .jltwpafaq-overview__go-pro a {
				color: #FCB92C;
				font-weight: 500;
			}
		</style>
	...
	public static function get_data() {
		return get_option( 'jltwpafaq_sheet_promo_data' );
	}
	...
	public function get_data_hash() {
		return get_option( 'jltwpafaq_sheet_promo_data_hash' );
	}
	...
	public function register_sync_hook() {
		$hook_action = 'jltwpafaq_sheet_promo_data_remote_sync';
		...
		register_deactivation_hook( JLTWPAFAQ_FILE, array( $this, 'clear_register_sync_hook' ) );
	}
	...
	public function clear_register_sync_hook() {
		wp_clear_scheduled_hook( 'jltwpafaq_sheet_promo_data_remote_sync' );
	}
	...
	public function sheet_data_remote_sync() {
		...
		if ( $force || $sheet_hash_data !== $sheet_data_hash ) {
			update_option( 'jltwpafaq_sheet_promo_data', $remote_data );
			update_option( 'jltwpafaq_sheet_promo_data_hash', $sheet_data_hash );
			do_action( 'jltwpafaq_sheet_promo_data_reset' );
		}
	}
	...
	public function get_sheet_promo_remote_data() {
		...
		$plugin_slug = Helper::jltwpafaq_slug_cleanup();
	...
	public function display_popup() {
		...
		<div class="jltwpafaq-popup jltwpafaq-upgrade-popup" id="jltwpafaq-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1" style="display: none;">

			<div class="jltwpafaq-popup-overlay"></div>

			<div class="jltwpafaq-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --jltwpafaq-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">

				<!-- close  -->
				<div class="jltwpafaq-popup-modal-close popup-dismiss">×</div>

				<!-- content section  -->
				<div class="jltwpafaq-popup-modal-footer">

					<!-- countdown  -->
					<div class="jltwpafaq-popup-countdown" style="display: none;">
						<span class="jltwpafaq-popup-countdown-text">Deal Ends In</span>
						<div class="jltwpafaq-popup-countdown-time">
							<div>
					...
					<!-- button  -->
					<a class="jltwpafaq-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $this->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			var $container = jQuery('#jltwpafaq-popup'),
				plugin_data = <?php echo json_encode( $this->get_sheet_promo_remote_data(), true ); ?>,
				events = {}; //Events

			// Update Counter
			function updateCounter(seconds) {
				const $counter = $container.find(".jltwpafaq-popup-countdown-time");
			...
			function initCounter(last_date) {
				$container.find(".jltwpafaq-popup-countdown-time").show();
				...
				if (result) {
					...
					$container.find(".jltwpafaq-popup-countdown").show(0);
				} else {
					trigger("countdownFinish", [plugin_data]);
					$container.find(".jltwpafaq-popup-countdown").hide(0);
				}
