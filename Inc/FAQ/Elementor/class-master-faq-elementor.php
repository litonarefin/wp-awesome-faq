<?php 
namespace JLTWPAFAQ\Inc\FAQ\Elementor;

use JLTWPAFAQ\Inc\FAQ\Elementor\Addon\Master_Accordion;
...
if( !class_exists('Master_Accordion_Elementor') ){
	...
	final class Master_Accordion_Elementor {
		...
		public function jltmaf_init_widgets() {
			...
			\Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Master_Accordion() );	
		...
		public function jltmaf_plugin_actions_links( $links ) {
			if ( is_admin() ) {					
				$links[] = '<a href="' . esc_url_raw('https://master-addons.com/contact-us') .'" target="_blank">' . esc_html__( 'Support', 'wp-awesome-faq' ) . '</a>';
		...
		public function jltmaf_admin_notice_missing_main_plugin() {
			...
			if ( $this->is_elementor_activated() ) {
				...
				$message = sprintf( 'Master Addons requires <b>Elementor</b> plugin to be active. Please activate Elementor to continue.', 'wp-awesome-faq' );
				$button_text = esc_html__( 'Activate Elementor', 'wp-awesome-faq' );
				...
				$message = sprintf( esc_html__( 'Master Addons requires %1$s"Elementor"%2$s plugin to be installed and activated. Please install Elementor to continue.', 'wp-awesome-faq' ), '<strong>', '</strong>' );
				$button_text = esc_html__( 'Install Elementor', 'wp-awesome-faq' );
			}
		...
		public function jltmaf_admin_notice_minimum_elementor_version() {
			...
			/* translators: 1: Plugin name 2: Elementor 3: Required Elementor version */
				esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.', 'wp-awesome-faq' ),
				'<strong>' . esc_html__( 'Master Addons for Elementor', 'wp-awesome-faq' ) . '</strong>',
				'<strong>' . esc_html__( 'Elementor', 'wp-awesome-faq' ) . '</strong>',
		...
		public function jltmaf_admin_notice_minimum_php_version() {
			...
			/* translators: 1: Plugin name 2: PHP 3: Required PHP version */
				esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.', 'wp-awesome-faq' ),
				'<strong>' . esc_html__( 'Master Addons for Elementor', 'wp-awesome-faq' ) . '</strong>',
				'<strong>' . esc_html__( 'PHP', 'wp-awesome-faq' ) . '</strong>',
