...
function jltmaf_load_admin_scripts() {
	...
	// Scripts
	wp_enqueue_style( 'fonticonpicker', JLTWPAFAQ_URL . '/assets/fonticonpicker/css/base/jquery.fonticonpicker.min.css', false, JLTWPAFAQ_VER );		
	wp_enqueue_style( 'fonticonpicker-grey', JLTWPAFAQ_URL . '/assets/fonticonpicker/css/themes/grey-theme/jquery.fonticonpicker.grey.min.css', false, JLTWPAFAQ_VER );
	wp_enqueue_style( 'font-awesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.12.0-2/css/all.min.css' );	

	wp_enqueue_script( 'fonticonpicker', JLTWPAFAQ_URL . '/assets/fonticonpicker/js/jquery.fonticonpicker.min.js', array( 'jquery'), JLTWPAFAQ_VER, true );	
	wp_enqueue_script( 'master-accordion-admin', JLTWPAFAQ_URL . '/assets/js/proscript.js', array('jquery', 'wp-color-picker'), JLTWPAFAQ_VER, true );

	$jltmaf_upgrade_pro = '<div class="jltmaf-text-small" style="padding-top:20px;"> Upgrade to  <a href="' . jltwpafaq_license_client()->get_upgrade_url() . '">Pro Version</a> unlock this feature.</div>';
...
function jltmaf_frontend_scripts() {
	wp_enqueue_style( 'master-accordion', JLTWPAFAQ_URL . '/assets/css/master-accordion.css', array(), JLTWPAFAQ_VER, 'all' );
	wp_enqueue_style( 'font-awesome', '//cdnjs.cloudflare.com/ajax/libs/font-awesome/5.12.0-2/css/all.min.css' );		
	wp_enqueue_script( 'master-accordion', JLTWPAFAQ_URL . '/assets/js/master-accordion.js', array('jquery'), JLTWPAFAQ_VER, true );
